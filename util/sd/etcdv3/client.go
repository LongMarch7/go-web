...
type Client interface {
	...
	SetKV(key string, value string) (error)

	GetKV(key string) ([]byte)
...
func (c *client) GetEntries(key string) ([]string, error) {
	...
	entries := make([]string, 0, len(resp.Kvs))
...
// set key.
func (c *client) SetKV(key string, value string) (error) {
...
// set key.
func (c *client) GetKV(key string) ([]byte) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
